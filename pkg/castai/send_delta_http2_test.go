...
func TestLogHTTP2RequestError(t *testing.T) {
	tests := []struct {
		...
		{
			name: "stream error surfaces error code and stream id",
		...
		{
			name: "goaway error surfaces error code and last stream id",
		...
		},
		{
			...
			wantNoFields: []string{"http2_stream_id", "http2_error_code", "http2_last_stream_id"},
		},
	}
